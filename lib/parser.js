...
class Parser {
  static parse(script) {
    ...
    function parseLines(startIndex, parent) {
      ...
      winston.info("start at", startIndex);
      for (_i = startIndex; _i < lines.length; _i++)
      {
        ...
        else if (tokens[0] == "citire")
        {
          let nextNode = tree.createNode({type: 'function', functionName: 'scan', parameters: _.takeRight(tokens, tokens.length-1)});
          node.next = nextNode;
          node = node.next;
        }
